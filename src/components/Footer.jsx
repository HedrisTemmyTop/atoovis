  ...
  const waitingListHandler = async () => {
    try {
      const response = await axios.post(
        "https://atoovis-be.herokuapp.com/waitlist",
        {
          email: email,
        }
      );
      if (response.status >= 200 && response.status < 300) {
        return alert(
          `Congratulations, Your E-mail (${email}) has been added to our waiting list `
        );
      }
    } catch (error) {
        ...
        <div className="main">
          ...
          <div>
            <p style={{ textAlign: "left" }}>
              Stay updated with our newsletter to get the best offer
            </p>
            <div className="mid">
              <div className="email">
                <input
              ...
              <button className="btn" onClick={waitingListHandler}>
                Subscribe
              </button>
            </div>
          </div>
          ...
          <div className="list">
            <ul>
              ...
              <li>
                {" "}
                <Link to="/vendor">Sell on Atoovis</Link>
              </li>
            </ul>
          </div>
